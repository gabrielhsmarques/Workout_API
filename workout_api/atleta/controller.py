...
async def post(
    ...
    if not categoria:
        raise HTTPException(
            ...
            detail=f'A Categoria {atleta_in.categoria.nome} nao foi encontrada.')
    ...
    if not centro_treinamento:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f'O centro de treinamento {atleta_in.centro_treinamento.nome} nao foi encontrado.')
    try:
...
async def query(id: UUID4, db_session: DataBaseDependency, atleta_up: AtletaUpdate = Body(...))->AtletaOut:
    ...
    if not atleta:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f'Atleta nao encontrada no id: {id}')
